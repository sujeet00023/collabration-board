...
export default function BoardPage() {
  ...
  const { activeBoard, fetchBoard, addColumn,
          moveCard, onlineUsers, isLoading }        = useBoardStore()
  ...
  // useChat lives HERE — not inside ChatSidebar
  // This means messages accumulate even when the sidebar is closed
  const { messages, sendMessage } = useChat(boardId)
  ...
  // Unread counter — increments when sidebar is closed and new messages arrive
  useEffect(() => {
  ...
  function openChat() {
    setChatOpen(true)
    setUnread(0)   // clear badge when opening
  }
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
              ...
              <div className="flex gap-4 p-4 items-start min-w-max min-h-full">
                ...
                <div className="w-72 flex-shrink-0">
                  {showAddCol ? (
                    ...
                    >
                      <input
                        autoFocus type="text" className="input text-sm mb-2"
                        placeholder="Column name..."
                        value={newColTitle}
                        onChange={e => setNewColTitle(e.target.value)}
                        onKeyDown={e => {
                          if (e.key === 'Escape') { setShowAddCol(false); setNewColTitle('') }
                        }}
                      />
        ...
        {/* Chat sidebar — conditionally RENDERED but messages state always alive above */}
        {chatOpen && (
          <ChatSidebar
            messages={messages}
            onSend={sendMessage}
