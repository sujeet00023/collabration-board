...
export function registerBoardHandlers(io: Server, socket: Socket): void {
  ...
  const normalizeRoom = (id: string) => id.toString().trim()
  // ── BOARD: JOIN ────────────────────────────────────────────
socket.on('board:join', async ({ boardId }) => {
  const room = normalizeRoom(boardId)
  ...
  try {
    ...
    io.to(room).emit('user:joined', {
      ...
      color,
      socketId: socket.id,
      onlineCount: RoomManager.getCount(room),
    })
  ...
  const room = normalizeRoom(boardId)
    ...
    socket.leave(room)
    RoomManager.leave(room, socket.id)
    io.to(room).emit('user:left', { userId, name, socketId: socket.id, onlineCount: RoomManager.getCount(room) })
  })
  ...
  const room = normalizeRoom(boardId)
    ...
    socket.to(room).emit('cursor:moved', { socketId: socket.id, userId, name, color, x, y })
  })

  // ── CURSOR: LEAVE (mouse left the board canvas) ────────────
  socket.on('cursor:leave', ({ boardId }: JoinBoardPayload) => {
    const room = normalizeRoom(boardId)
    socket.to(room).emit('cursor:left', { socketId: socket.id, userId })
  })
 ...
 socket.on('card:moved', async (payload: CardMovedPayload) => {
  const room = normalizeRoom(payload.boardId)

  const { cardId, fromColumnId, toColumnId, newOrder } = payload
  ...
  try {
    ...
    const cardIdx = fromColumn.cards.findIndex(
  ...
  // ── CARD: CREATED ──────────────────────────────────────────
socket.on('card:created', async ({ boardId, columnId, title }) => {
  const room = normalizeRoom(boardId)
  ...
  // ── CARD: UPDATED ──────────────────────────────────────────
  socket.on('card:updated', async ({ boardId, columnId, cardId, changes }) => {
  const room = normalizeRoom(boardId)
  ...
  // ── CARD: DELETED ──────────────────────────────────────────
 socket.on('card:deleted', async ({ boardId, columnId, cardId }) => {
  const room = normalizeRoom(boardId)
  ...
  // ── COLUMN: CREATED ────────────────────────────────────────
socket.on('column:created', async ({ boardId, title }) => {
  const room = normalizeRoom(boardId)
  ...
  try {
    ...
    io.to(room).emit('column:created', {
  ...
  // ── COLUMN: DELETED ────────────────────────────────────────
 socket.on('column:deleted', async ({ boardId, columnId }) => {
  const room = normalizeRoom(boardId)
  ...
  try {
    ...
    io.to(room).emit('column:deleted', {
      columnId,
      deletedBy: { userId, name },
    })
  } catch {
